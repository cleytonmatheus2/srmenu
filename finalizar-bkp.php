  ...
  <div class="accordion-item">
    ...
    <div id="collapseOne" class="accordion-collapse collapse show" aria-labelledby="headingOne" data-bs-parent="#accordionExample">
     ...
     <?php } ?>
       ...
       <hr>
       <div><b>Confirmar Identificação?</b></div>
       <hr>
       <div class="row">
        <div class="col-6">
...
$query8 = $pdo->query("SELECT * FROM carrinho where sessao = '$sessao' and id_sabor = 0");
$res8 = $query8->fetchAll(PDO::FETCH_ASSOC);
$total_reg8 = @count($res8);
  for($i8=0; $i8 < $total_reg8; $i8++){
    foreach ($res8[$i8] as $key => $value){}  
    $produto = $res8[$i8]['produto'];
    $id_car = $res8[$i8]['id'];
    $quantidade = $res8[$i8]['quantidade'];
    $variacao = $res8[$i8]['variacao'];

    $query2 = $pdo->query("SELECT DISTINCT * FROM variacoes where id = '$variacao'");
...
$query9 = $pdo->query("SELECT * FROM produtos where id = '$produto'");
$res9 = $query9->fetchAll(PDO::FETCH_ASSOC);
$total_reg9 = @count($res9);
  for($i9=0; $i9 < $total_reg9; $i9++){
    foreach ($res9[$i9] as $key => $value){}  
    $idcarrrinho = $res9[$i9]['id'];
    $nomeproduto = $res9[$i9]['nome']; 
...
<span style="padding-left: 25px;"><?php echo $quantidade; ?> x <?php echo $nomeproduto; ?> <?php echo $sigla_variacao; ?></span>
...
<?php
 ...
 $query7 = $pdo->query("SELECT * FROM temp where carrinho = '$id_car' and tabela = 'adicionais'");//735
...
if ($total_reg7 >= 1 ) { ?> 
 ...
 <?php  } else { 
   ...
   for($i7=0; $i7 < $total_reg7; $i7++){
    foreach ($res7[$i7] as $key => $value){}  
        $id_item = $res7[$i7]['id_item']; //15,16,17
        $tabela = $res7[$i7]['tabela']; 
...
$total_reg3 = @count($res3);
   for($i3=0; $i3 < $total_reg3; $i3++){
    foreach ($res3[$i3] as $key => $value){}  
     $id_item_adc = $res3[$i3]['id']; 
        $item_adc = $res3[$i3]['nome']; 
...
?>
 ...
 <?php
        ...
        $query4 = $pdo->query("SELECT * FROM temp where carrinho = '$id_car' and tabela = 'ingredientes'");
...
if ($total_reg4 >= 1 ) { ?> 
 ...
 <?php  } else { 
  ...
  for($i4=0; $i4 < $total_reg4; $i4++){
    foreach ($res4[$i4] as $key => $value){}  
        $id_item_ingr_temp = $res4[$i4]['id_item']; 


      $query5 = $pdo->query("SELECT * FROM ingredientes where id = '$id_item_ingr_temp'");
      $res5 = $query5->fetchAll(PDO::FETCH_ASSOC);
      $total_reg5 = @count($res5);

  for($i5=0; $i5 < $total_reg5; $i5++){
    foreach ($res5[$i5] as $key => $value){}  
        $id_item_ingr = $res5[$i5]['id']; 
        $item_ingr = $res5[$i5]['nome']; 
...
var xxx = document.getElementById("resultzap");

var texxxx  = xxx.textContent || xxx.innerText;
...
`);
   ...
   var link = 'https://wa.me/5511993195502?text='+texto; 
    
   jQuery('#testbotao').prop('href',link);
